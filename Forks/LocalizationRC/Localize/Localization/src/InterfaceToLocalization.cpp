...
InterfaceToLocalization::InterfaceToLocalization(Map * map, int fieldOfVision, PlayerClient * robot) {
	mc = new MonteCarlo(map);
	mc->setRandomCoefficients(Utils::toRadians(10), 20, Utils::toRadians(20), 40);
...
bool InterfaceToLocalization::blobOnTopOf(player_blobfinder_blob top,
	...
	return (top.left < bottom.right && top.right > bottom.left && top.bottom
			< bottom.top + 3 && top.bottom + 10 > bottom.top);
}
...
bool InterfaceToLocalization::positionEqual(Position p1, Position p2) {
	...
	return (absx < 1 && absy < 1 && Utils::toRadians(abst) < 1);
}
