...
// m/s -> cm
static const double VELS[] = {0.10, 0.20, 0.30};
static const double DIST[] = {10.0, 20.0, 40.0, 80.0};
// rad/s -> deg
static const double RADS[] = {1.00, 2.00, 3.00};
static const double DEGS[] = {22.5, 45.0, 90.0, 180.0};

static double __computeLinearMove(double v) {
	if (v > 0) {
		if      (v <= VELS[0])  v = DIST[0];
		else if (v <= VELS[1])  v = DIST[1];
		else if (v <= VELS[2])  v = DIST[2];
		else                    v = DIST[4];
	} else if (v < 0) {
		if      (v >= -VELS[0]) v = -DIST[0];
		else if (v >= -VELS[1]) v = -DIST[1];
...
static double __computeAngularMove(double v) {
	if (v > 0) {
		if      (v <= RADS[0])  v = DEGS[0];
		else if (v <= RADS[1])  v = DEGS[1];
		else if (v <= RADS[2])  v = DEGS[2];
		else                    v = DEGS[4];
	} else if (v < 0) {
		if      (v >= -RADS[0]) v = -DEGS[0];
		else if (v >= -RADS[1]) v = -DEGS[1];
 ...
 mMyMap(0), mLocalization(0)
{
	...
	// Set up the map to be used for localization.
	if ((mMyMap = new Map(600, 580)) == 0) {
		throw runtime_error("failed to allocate memory for map");
	}
	mMyMap->addMarker(MapMarker("p/b", 100, 0));
	mMyMap->addMarker(MapMarker("p/y", 400, 0));
	mMyMap->addMarker(MapMarker("p/b", 500, 150));
	mMyMap->addMarker(MapMarker("p/o", 500, 450));
	mMyMap->addMarker(MapMarker("b/p", 300, 567));
	mMyMap->addMarker(MapMarker("o/p", 150, 300));
	mMyMap->addMarker(MapMarker("y/p", 0, 100));
	mMyMap->addMarker(MapMarker("pink", 100, 0));
	mMyMap->addMarker(MapMarker("pink", 400, 0));
	mMyMap->addMarker(MapMarker("pink", 500, 150));
	mMyMap->addMarker(MapMarker("pink", 500, 450 )); 
	mMyMap->addMarker(MapMarker("pink", 300, 567));
	mMyMap->addMarker(MapMarker("pink", 150, 300));
	mMyMap->addMarker(MapMarker("pink", 0, 100));

	// Set up the localization object.
	try {
		if ((mLocalization = new Surveyor(mMyMap, &mPlayerClient)) == 0) {
			throw runtime_error("failed to allocate memory for localization");
		}
	} catch (...) {
		cerr << "Robot::Robot() - failed to create localization object" << endl;;
		throw;
	}
}
...
void Robot::do_state_action_moving()
{
	...
	try {
		xv = __computeLinearMove(xv);
		yv = __computeLinearMove(yv);
		av = __computeAngularMove(av);
		mLocalization->move(Position(xv, yv, av));
	} catch (PlayerError) {
		stringstream oss;
...
void Robot::do_state_action_pose()
{
	...
	try {
		Position pos = mLocalization->getPosition();
		xp = pos.getX();
		yp = pos.getY();
		ap = pos.getTheta();
		confidence = mLocalization->getConfidence();
	} catch (PlayerError) {
		stringstream oss;
